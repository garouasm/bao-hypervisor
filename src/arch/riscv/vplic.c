...
static void vplic_emul_enbl_access(emul_access_t *acc)
{
    ...
    if (vplic_vcntxt_valid(cpu.vcpu, vcntxt_id)) {
        for (int i = 0; i < 32; i++) {
            if (acc->write) {
                vplic_set_enbl(cpu.vcpu, vcntxt_id, first_int + i,
                               val & (1U << i));
            } else {
                val |= (vplic_get_enbl(cpu.vcpu, vcntxt_id, first_int + i)
                            ? (1U << i)
                            : 0);
            }
        }
        ...
        case offsetof(plic_hart_t, threshold):
            if (acc->write) {
                ...
                vcpu_writereg(cpu.vcpu, acc->reg,
                              vplic_get_theshold(cpu.vcpu, vcntxt));
            }
            break;
        case offsetof(plic_hart_t, claim):
            if (acc->write) {
                vplic_complete(cpu.vcpu, vcntxt,
                               vcpu_readreg(cpu.vcpu, acc->reg));
            } else {
                vcpu_writereg(cpu.vcpu, acc->reg,
                              vplic_claim(cpu.vcpu, vcntxt));
            }
            break;
    }
